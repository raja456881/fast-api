...
class Business(Model):
    id = fields.IntField(pk=True, index=True)  
    business_name=fields.CharField(max_length=200, null=False, unique=True)
...
class Product(Model):
    ...
    offer_expiration_data=fields.DateField(defaul=datetime.utcnow)
...
user_pydantic_in=pydantic_model_creator(User, name="UserIn", exclude=("is_verified","join_data"), exclude_readonly=True,)
...
product_pydantic_in=pydantic_model_creator(Product, name="ProductIn", exclude=("percentage_discount", "id"))
